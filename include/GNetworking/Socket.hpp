...
public:
  ...
  SOCKET SetSocket(const SOCKET &_sock, const int &_family, const int &_type,
                   const int &_protocol);
  SOCKET CreateSocket(const int &_family, const int &_type,
                      const int &_protocol);
  ...
  SOCKET Accept(Socket &_connectedSock, const std::string &_addr,
